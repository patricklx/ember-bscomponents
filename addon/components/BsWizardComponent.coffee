BsWizardStep = ItemView.extend(ItemSelection, NavItem,
...
BsWizardSteps = ItemsView.extend(Nav,
  navType: 'pills'
  classNames: ['wizard-steps']
  itemViewClass: BsWizardStep
...
BsWizardStepPane = ItemPaneView.extend(
)

BsWizardStepsPanes = ItemsPanesView.extend(
  classNames: ['wizard-panes']
  itemViewClass: BsWizardStepPane
)

BsWizardComponent = Ember.ContainerView.extend(Ember.TargetActionSupport,
  classNames: ['wizard']
  ...
  steps: BsWizardSteps.extend(
    contentBinding: 'parentView.content'
    selectedBinding: 'parentView.selected'
  )

  panes: BsWizardStepsPanes.extend(
    contentBinding: 'parentView.content'
  )
  ...
  controls: Ember.ContainerView.extend (
    ...
    finish: BsButtonComponent.extend(
      #TODO: This is a hack until it will be possible to extend from component as it looses the template association
      #see https://github.com/emberjs/ember.js/issues/3376
      layoutName: 'components/bs-button'
      title: 'Finish'
...
BsWizardComponent = BsWizardComponent.reopenClass(
  build: (options) ->
...
Ember.Handlebars.helper 'bs-wizard', BsWizardComponent
