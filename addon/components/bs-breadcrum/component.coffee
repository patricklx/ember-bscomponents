...
###
BsBreadcrumbsItem = ItemView.extend(
    tagName: ['li']
    classNameBindings: ["isActive:active"]
    template: Ember.HTMLBars.compile '{{#unless view.isActive}}{{#if view.content.model}}{{#link-to view.content.route model.id}}{{view.content.name}}{{/link-to}}{{else}}{{#link-to view.content.route}}{{view.content.name}}{{/link-to}}{{/if}}{{else}}{{view.content.name}}{{/unless}}'
...
BsBreadcrumbs = ItemsView.extend(WithRouter,
    tagName: ['ol']
